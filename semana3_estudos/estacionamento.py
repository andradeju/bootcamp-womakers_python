...
class Estacionamento:
    ...
    def estacionar_carro(self, carro: Carro):
      ...
      if id_da_proxima_vaga is None:
        ...
        self.total_vagas_livres_carro -= 1 #reduz o nº de vagas livres
      else:
    ...
    def estacionar_moto(self, moto: Moto):
      if moto.estacionado is True:
        raise ValueError(f'A moto {moto.placa} já está no estacionamento')
      id_da_proxima_vaga, tipo = self.buscar_id_da_proxima_vaga_livre('moto') #gera o id d próxima vaga

      if id_da_proxima_vaga is None:
        raise ValueError(f'Não há mais vgas de moto disponiveis no estacionamento')
        ...
        self.total_vagas_livres_moto -= 1 #reduz o numero de vagas livres  
      elif id_da_proxima_vaga is not None and tipo is 'carro':
        vaga = self.vagas_carro[id_da_proxima_vaga] 
        vaga.ocupar(moto.placa)
        moto.estacionar()
        self.moto_para_vaga[moto.placa] = vaga.id
        self.total_vagas_livres_carro -= 1 #reduz o numero de vagas livres             
      else:
    ...
    def estado_do_estacionamento(self):
      ...
      estado += f' Numero de carros estacionados: {num_carros_estacionados}\n'
      estado += f' Numero de motos estacionadas: {num_motos_estacionadas}\n'
