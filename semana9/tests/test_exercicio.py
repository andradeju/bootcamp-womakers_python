...
def admin_command(command, sudo=True):
  if not isinstance(command, list):
    raise TypeError(f"esperava que o command fosse uma lista, mas recebi um {type(command)}")
...
class TestAdminCommand:
  ...
  def test_non_list_commands(self):
        ...
        assert error.value.args[0] == "esperava que o command fosse uma lista, mas recebi um <class 'str'>"    